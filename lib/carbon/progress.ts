...
export const ACTIVITY_STORAGE_KEY = "carbon-compass:activities:v1";
...
export function loadActivityLog(storage = safeStorage()): ActivityLogEntry[] {
  ...
  try {
    ...
    const parsed = activityPayloadSchema.safeParse(JSON.parse(raw));
...
export function saveActivityLog(entries: ActivityLogEntry[], storage = safeStorage()): void {
  if (!storage) {
    return;
  }
