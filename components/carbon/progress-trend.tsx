import { bestMonthlyImprovement, progressHistoryTextSummary } from "@/lib/carbon/progress";
...
export function ProgressTrend({ entries }: ProgressTrendProps) {
  const maxMonthlyTotal = Math.max(...entries.map((entry) => entry.monthlyTotalKgCO2e), 1);
  ...
  return (
    ...
    >
      <h2 className="text-2xl font-semibold text-slate-950">Progress trend</h2>
        ...
        {entries.map((entry) => (
          <div key={entry.id} className="grid gap-2">
            <div className="flex items-center justify-between gap-3 text-sm">
              <span className="font-medium text-slate-700">{shortDate(entry.recordedAt)}</span>
              <span className="text-slate-600">{kg(entry.monthlyTotalKgCO2e)}</span>
            </div>
            <div className="h-3 overflow-hidden rounded-full bg-slate-100">
              <div
                className="h-full rounded-full bg-emerald-700"
                style={{
                  width: `${Math.max((entry.monthlyTotalKgCO2e / maxMonthlyTotal) * 100, 8)}%`,
                }}
              />
            </div>
          </div>
      ...
      >
        {entries.map((entry) => (
          <li key={entry.id}>
            {shortDate(entry.recordedAt)}: {kg(entry.monthlyTotalKgCO2e)}, eco score{" "}
            {entry.ecoScore}/100, top source {entry.topCategory}.
          </li>
        ))}
      </ul>
    </section>
