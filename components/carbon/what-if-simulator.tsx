  ...
  return (
    ...
    >
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-slate-950">What-if simulator</h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-700">
            Compare deterministic before and after estimates for practical action ideas. Each saving
            comes from the same carbon engine and factor set as the dashboard.
          </p>
        </div>
      </div>
      ...
      <ul className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {simulations.map((simulation) => {
          ...
          return (
            <li key={action.id} className="rounded-md border border-slate-200 bg-slate-50 p-4">
              <p className="text-sm font-semibold capitalize text-emerald-700">{action.category}</p>
              ...
              <p className="mt-4 text-xs leading-5 text-slate-600">{action.factorNote}</p>
            </li>
          );
        })}
      </ul>
